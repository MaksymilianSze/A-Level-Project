...
class FrameHolder(tk.Tk): #Inherits from Tk
    ...
    def GetName(self, Type, ID):
        ...
        if Type == "Student":
            ...
            cursor.execute("SELECT TeacherName FROM teachers WHERE TeacherID = %s", (ID,) )
            FoundName = cursor.fetchone()[0]
            return FoundName
...
class StudentDetentionFrame(tk.Frame, FrameHolder):
    def __init__(self, parent, controller): 
        ...
        self.DefaultEmailText = tk.StringVar(self, value="Enter Email Here")
        ...
        self.DetentionTree["columns"]=("0","1","2","3")
        self.DetentionTree.column("0", width=120, minwidth=100, stretch=tk.NO)
        self.DetentionTree.column("1", width=120, minwidth=50, stretch=tk.NO)
        ...
        self.DetentionTree.heading("0", text="Date",anchor=tk.W)
        self.DetentionTree.heading("1", text="Room Number",anchor=tk.W)
        self.DetentionTree.heading("2", text="Length",anchor=tk.W)
        self.DetentionTree.heading("3", text="Teacher",anchor=tk.W)

        self.DetentionTree.grid(columnspan=2,row=1)
        self.DetentionTree['show'] = 'headings'

        self.EmailNotificationsEntry = tk.Entry(self, textvariable=self.DefaultEmailText)
        self.EmailNotificationsEntry.grid(columnspan=2)

        self.AddEmailButton = tk.Button(self, text="Add Email",command=lambda:  self.AddEmail( self.EmailNotificationsEntry.get(), controller.SharedData["UsernameHolder"].get() ) )
        self.AddEmailButton.grid(row=3, column=0, sticky="E")

        self.RemoveEmailButton = tk.Button(self, text="Remove Email",command=lambda:  self.RemoveEmail( controller.SharedData["UsernameHolder"].get() ) )
        self.RemoveEmailButton.grid(row=3, column=1, sticky="W")

        self.RefreshDetentions = tk.Button(self, text="Refresh Detentions",command=lambda:  self.GetDetentions( controller.GetID( "Student", self.controller.SharedData["UsernameHolder"].get() ), controller ) )
    ...
    def AddEmail(self, Email, Student):
        cursor.execute("UPDATE students SET StudentEmail = %s WHERE StudentName = %s", (Email, Student) )
        db.commit()

    def RemoveEmail(self, Student):
        cursor.execute("UPDATE students SET StudentEmail = %s WHERE StudentName = %s", (None, Student) )


    def GetDetentions(self, StudentID, controller):
        self.controller = controller
        cursor.execute( "SELECT TeacherID FROM detentions WHERE StudentID = %s", (StudentID,) )
        ...
        cursor.execute( "SELECT DetentionDateStamp, Location, Length FROM detentions WHERE StudentID = %s ", (StudentID,) )
        DetentionInfo = cursor.fetchall()

       
        for x in ListOfIds:
            ListOfNames.append( controller.GetName("Teacher", x) )


        AllInfo = tuple( zip( *zip(*DetentionInfo), ListOfNames ) )
...
class TeacherSetDetentionFrame(TeacherDetentionFrame):
    ...
    def SetDetention(self, Student, Teacher, DetentionDate, Length, Location, Ground, Description, controller): 
        ...
        cursor.execute("SELECT StudentEmail FROM students WHERE StudentName = %s", (Student,))
        ...
        if Email:
            ...
            """ .format(Teacher, DetentionDate, Location) )
            ...
            with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
                ...
                server.sendmail( "detentionnotification@gmail.com", "{0}".format(Email), message.as_string() )
